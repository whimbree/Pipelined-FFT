...
def main():
    actual_output = pd.read_csv("tb_testOutput.csv", header=None, sep=r",")
    expected_output = pd.read_csv("tb_expectedOutput.csv",
                                  header=None,
                                  sep=r",")

    difference = (actual_output - expected_output).abs()

    percent_error = compute_percent_error(actual_output.values,
                                          expected_output.values)

    percent_error_tolerance = 20
...
def compute_percent_error(actual: np.array,
                          expected: np.array) -> pd.DataFrame:
